...
class UAVDT(data.Dataset):
    ...
    mean = np.array([0.37296916, 0.38584609, 0.38465398],
                    dtype=np.float32).reshape(1, 1, 3)
    std = np.array([0.17721877, 0.16971162, 0.17878106],
                   dtype=np.float32).reshape(1, 1, 3)

    def __init__(self, opt, split):
        super(UAVDT, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'uav')
        self.img_dir = os.path.join(self.data_dir, 'images')

        print('load cluster region')
        if split == 'val':
