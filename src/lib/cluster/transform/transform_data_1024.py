...
def fill_img(img):
    ...
    w_fill = max(824 - w, 0)
    w_fill = w_fill + 200 if w_fill>0 else 0
    h_fill = max(824 - h, 0)
    h_fill = h_fill + 200 if h_fill>0 else 0
...
def cutfill_1024(img, target):
    ...
    if w<=1224 and h<=1224:
        ...
        # 裁剪两块再填充
    elif (w<=1224 or h<=1224) and (w>1224 or h>1224):
        if w > 1224:
            # 分割为512
...
if __name__ == "__main__":
    ...
    # read_im_dir = vis_dir + '\\cluster_train'
    ...
    # save_im_dir = os.path.join(vis_dir,'cluster_transform_1024-824')
    # save_ann_dir = os.path.join(vis_dir,'cluster_transform_annotations_1024-824')

    data_dir = r'C:\Users\10295\CV\ctNet\data'
    vis_dir = os.path.join(data_dir,'visdrone2019')
    coco_dir = os.path.join(data_dir,'coco2017')
    read_im_dir = vis_dir + '\\cluster_val'
    visClu_anno_file = os.path.join(vis_dir,'annotations','cluster_val.json')
    coco_anno_file = os.path.join(coco_dir,'annotations','instances_train2017.json')
    save_im_dir = os.path.join(vis_dir,'cluster_transform_val_1024-824')
    save_ann_dir = os.path.join(vis_dir,'cluster_transform_val_annotations_1024-824')

    # 按照dst分布转换src分布
    # sm = MonotonicityScaleMatch(src_anno_file, dst_anno_file)
    transform = GaussTransfrom(mu=98, sigma=5, scale_range=(0., 3.))
    # transform = Compose([sm])
    dataset = COCODataset(visClu_anno_file, read_im_dir, True)
    for ind, (img, target, img_info) in  enumerate(dataset):
        ...
        imgs, bboxss, w_fills, h_fills = cutfill_1024(img, target)
