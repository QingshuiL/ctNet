...
class ScaleMatch(object):
    ...
    def _get_distribute(annotations, bins=100, except_rate=0.1, use_log_bins=False, mu_sigma=(-1, -1)):
        ...
        """
        annos = [anno for anno in annotations if not anno['iscrowd']]                    # 剔除 iscrowd 非0项
        if len(annos) > 0 and 'ignore' in annos[0]:                                      # 剔除 ignore 非0项
            annos = [anno for anno in annos if not anno['ignore']]
        sizes = np.sqrt(np.array([anno['bbox'][2] * anno['bbox'][3] for anno in annos])) # 计算ann绝对尺寸
        sizes = sizes[sizes > 0]
        ...
        if except_rate > 0:
            c, s = np.histogram(hist_sizes, bins=bins-2)         # 产生98组 c为每组数量 s为每组上下界
...
class MonotonicityScaleMatch(object):
    def __init__(self, 
        ...
        dst_distri_cumsum = np.cumsum(dst_distri)           # 累加序列得到累积概率
    ...
    def __call__(self, image, target):                    # 使得类实例化后可以向函数一样调用
        if len(target.bbox) == 0:
            return self.default_scale_deal(image, target)
