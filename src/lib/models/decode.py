...
def _nms(heat, kernel=3):
    ...
    hmax = nn.functional.max_pool2d(
        heat, (kernel, kernel), stride=1, padding=pad) # 筛选3x3区域的最大值
    keep = (hmax == heat).float() # 最大值点标记1 其他0
    return heat * keep            # 保留最大值
...
def _topk(scores, K=40):
    ...
    topk_inds = topk_inds % (height * width)  # batch*cat*K
    # topk_ys   = (topk_inds / width).int().float()
    topk_ys = torch.floor_divide(topk_inds, width).float()
    topk_xs   = (topk_inds % width).int().float()   # 得到横 纵坐标
      
    topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K) # 再次筛选 得出batch x K个目标
    # topk_clses = (topk_ind / K).int()  # topk_clses为 (batch*cat*K) 
...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()                     # batch 类别 宽 高

    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    heat = _nms(heat)
      
    scores, inds, clses, ys, xs = _topk(heat, K=K)  # 得到每一个batch的 scores前K的目标信息 batch x K; 
    if reg is not None:                             # ys xs 中心点的横纵坐标
      reg = _transpose_and_gather_feat(reg, inds) # reg: # 从原始 1*10*256*256中，选出K个目标的偏移
    ...
    wh = _transpose_and_gather_feat(wh, inds) # 选取K个
    if cat_spec_wh:
    ...
    bboxes = torch.cat([xs - wh[..., 0:1] / 2, 
                        ys - wh[..., 1:2] / 2,
                        xs + wh[..., 0:1] / 2, 
                        ys + wh[..., 1:2] / 2], dim=2)  # torch.cat 拼接四个点
    detections = torch.cat([bboxes, scores, clses], dim=2) # 1x200x6 x1,y1,x2,y2,scores,clses
    # 筛选出值得聚类的区域
    return detections
